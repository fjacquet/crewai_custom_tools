...
Ruff runs in CI as its own `lint` job (`ruff check src tests scripts`, must be clean) and ships in `[dev]`, so `uv run ruff check src tests scripts` locally uses the same version. Ruff is **configured**, not left on defaults: `select = [E, W, I, UP, B, C4, SIM, RUF]` and `line-length = 120` (measured on `src tests scripts`: p99 = 99 chars, 7 lines over 120, but 1279 over ruff's default 88). `RUF001`/`RUF002`/`RUF003` are disabled — this is a French-language project and the typographic apostrophe is deliberate. `models/__init__.py` and `models/reports/__init__.py` ignore `F403` (pure re-export aggregators); `gramps_generated.py` ignores `E501` (generated file, ADR-0004); `__all__` ignores `RUF022` (grouped by category on purpose). The sibling `genecrew` repo now carries **the same config** (same `select`, same `line-length`, same `RUF001-003` ignores), so a green `ruff check` means the same thing on both sides — that was not true before 2026-07-22, when genecrew ran on ruff's four-rule default.
