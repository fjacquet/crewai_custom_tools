...
class RegistrySearchInput(BaseModel):
    ...
    query: str = Field(..., description="A SIREN (9-digit number) or free-text company name to search in the public French register.")
...
class FrenchRegistryTool(BaseTool):
    ...
    def _run(self, query: str) -> str:
        ...
        headers = {
            "User-Agent": "crew-custom-tools/0.1.0 (Enterprise Multi-Agent OSINT)"
        }
        ...
        if not results:
            return json.dumps({"error": f"No French corporate registry results found for query: {query}"})
