...
class FREDMacroTool(BaseTool):
    ...
    def _run(self, indicator: str) -> str:
        ...
        if not api_key:
            return json.dumps({"error": "FRED_API_KEY environment variable not configured"})
        ...
        if not series_id:
            return json.dumps({"error": f"Invalid indicator: {indicator}. Supported options: {list(series_map.keys())}"})
        ...
        if not observations:
            return json.dumps({"error": f"No observations returned for FRED series: {series_id}"})
...
class AlphaVantageOverviewTool(BaseTool):
    ...
    def _run(self, ticker: str) -> str:
        ...
        if "Error Message" in data:
            return json.dumps({"error": f"Alpha Vantage error: {data['Error Message']}"})
        if "Note" in data:
            return json.dumps({"error": f"Alpha Vantage rate limit message: {data['Note']}"})
